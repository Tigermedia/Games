...
class EDR_CSV_Handler {
     ...
     * Parse CSV file
     *
     * @param string $file_path Path to CSV file or HTTP URL
     * @return array|WP_Error Parsed CSV data or error
     */
    public static function parse_csv($file_path) {
        // Check if it's a URL
        $is_url = (strpos($file_path, 'http://') === 0 || strpos($file_path, 'https://') === 0);

        if ($is_url) {
            // Handle HTTP URL
            return self::parse_csv_from_url($file_path);
        }
    ...
    /**
     * Parse CSV from HTTP URL
     *
     * @param string $url URL to CSV file
     * @return array|WP_Error Parsed CSV data or error
     */
    private static function parse_csv_from_url($url) {
        // Try to get from cache first
        $cache_key = 'edr_csv_' . md5($url);
        $cached = get_transient($cache_key);

        if ($cached !== false) {
            EDR_Core::log('CSV loaded from cache (URL)', $url);
            return $cached;
        }
        ...
        if (is_wp_error($response)) {
            EDR_Core::log('Failed to fetch CSV from URL', array(
                'url' => $url,
                'error' => $response->get_error_message()
            ));
            return new WP_Error('fetch_failed', sprintf(
                __('Failed to fetch CSV from URL: %s', 'elementor-dynamic-redirect'),
        ...
        if ($http_code !== 200) {
            EDR_Core::log('HTTP error fetching CSV', array(
                'url' => $url,
                'code' => $http_code
            ));
            return new WP_Error('http_error', sprintf(
                __('HTTP error %d when fetching CSV', 'elementor-dynamic-redirect'),
                $http_code
            ));
        }
        ...
        if (empty($csv_content)) {
            return new WP_Error('empty_response', __('Empty response from CSV URL', 'elementor-dynamic-redirect'));
        }
        ...
        EDR_Core::log('CSV parsed and cached from URL', array(
            'url' => $url,
            'rows' => count($csv_data)
        ));
    ...
    public static function validate_csv_structure($file_path, $required_columns = array()) {
        // Try to parse the CSV (works for both URLs and file paths)
     ...
     * Get CSV info
     *
     * @param string $file_path Path to file or HTTP URL
    ...
    public static function get_csv_info($file_path) {
        ...
        if (!$is_url && !file_exists($file_path)) {
            return array(
                'exists' => false,
                'size' => 0,
                'modified' => '',
        ...
        if ($is_url) {
            return array(
                ...
                'modified' => '', // Modified date not available for URLs
                'rows' => $row_count,
                'path' => $file_path,
                'is_url' => true,
            );
